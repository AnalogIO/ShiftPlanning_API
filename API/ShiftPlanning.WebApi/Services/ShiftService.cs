...
using ShiftPlanning.WebApi.Helpers;
...
namespace ShiftPlanning.WebApi.Services
{
    ...
    public class ShiftService : IShiftService
    {
        ...
        public Shift AddEmployeesToShift(int shiftId, int organizationId, AddEmployeesDTO employeesDto)
        {
            var shift = _shiftRepository.Read(shiftId, organizationId);
            if (shift == null) throw new ObjectNotFoundException("Could not find a shift corresponding to the given id");
            if (shift.Employee_.Any(x => employeesDto.EmployeeIds.Contains(x.Id))) throw new ForbiddenException("You cannot add employees to a shift if they are already added to the shift");
            var employees =
        ...
        public Shift CreateShiftOutsideSchedule(CreateShiftOutsideScheduleDTO shiftDto, Organization organization)
        {
            ...
            var shift = new Shift { Start = start, End = end, CheckIns = new List<CheckIn>(), Employee_ = employees, Organization = organization };
        ...
        {/*
            if(((string)_contextAccessor.HttpContext.Items["Audit"]).Equals("Application"))
            {
        ...
        public Shift UpdateShift(int shiftId, int organizationId, UpdateShiftDTO updateShiftDto)
        {
            ...
            var employeesToRemove = shift.Employee_.Where(e => !updateShiftDto.EmployeeIds.Contains(e.Id)).ToList();
            ...
            foreach(var employee in employeesToRemove)
            {
                ...
                employee.Shift_.Remove(shift);
            }

            shift.Start = start;
            shift.End = end;
            shift.Employee_ = employees;
        ...
        public Shift PatchShift(int shiftId, int organizationId, PatchShiftDTO patchShiftDto)
        {
            ...
            if (patchShiftDto.EmployeeIds != null)
            {
                ...
                var employeesToRemove = shift.Employee_.Where(e => !patchShiftDto.EmployeeIds.Contains(e.Id)).ToList();
