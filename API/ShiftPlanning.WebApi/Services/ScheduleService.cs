...
namespace ShiftPlanning.WebApi.Services
{
    public class ScheduleService : IScheduleService
    {
        ...
        public ScheduledShift CreateScheduledShift(CreateScheduledShiftDTO scheduledShiftDto, Employee employee,
            int scheduleId)
        {
            var schedule = _scheduleRepository.Read(scheduleId, employee.Organization.Id);
            if (schedule == null)
                throw new ObjectNotFoundException("Could not find a schedule corresponding to the given id");
            var employees = _employeeRepository.ReadFromOrganization(employee.Organization.Id)
                .Where(x => scheduledShiftDto.EmployeeIds.Contains(x.Id)).ToList();
            var scheduledShift = new ScheduledShift
            {
                Day = scheduledShiftDto.Day, Start = TimeSpan.Parse(scheduledShiftDto.Start),
                End = TimeSpan.Parse(scheduledShiftDto.End), Schedule = schedule,
                MaxOnShift = scheduledShiftDto.MaxOnShift, MinOnShift = scheduledShiftDto.MinOnShift
            };
            scheduledShift.EmployeeAssignments =
                employees.Select(
                        e => new EmployeeAssignment { Employee = e, ScheduledShift = scheduledShift, IsLocked = false })
                    .ToList();
            schedule.ScheduledShifts.Add(scheduledShift);
            return _scheduleRepository.Update(schedule) > 0 ? scheduledShift : null;
        }

        public IEnumerable<ScheduledShift> CreateScheduledShifts(
            IEnumerable<CreateScheduledShiftDTO> scheduledShiftsDto, Employee employee, int scheduleId)
        {
            var schedule = _scheduleRepository.Read(scheduleId, employee.Organization.Id);
            if (schedule == null)
                throw new ObjectNotFoundException("Could not find a schedule corresponding to the given id");
            ...
            foreach (CreateScheduledShiftDTO scheduledShiftDto in scheduledShiftsDto)
            {
                ...
                scheduledShift.EmployeeAssignments =
                    employees.Select(
                            e => new EmployeeAssignment
                                { Employee = e, ScheduledShift = scheduledShift, IsLocked = false })
                        .ToList();
        ...
        public ScheduledShift UpdateScheduledShift(int scheduledShiftId, int scheduleId,
            UpdateScheduledShiftDTO scheduledShiftDto, Employee employee)
        {
            ...
            foreach (var emp in dbScheduledShift.EmployeeAssignments.Select(e => e.Employee).ToList())
            {
                emp.EmployeeAssignments =
                    emp.EmployeeAssignments
                        .Where(ea => ea.ScheduledShift != null && ea.ScheduledShift.Id != dbScheduledShift.Id).ToList();
            }
            ...
            dbScheduledShift.EmployeeAssignments =
                employees.Select(
                    e => new EmployeeAssignment
                    {
                        Employee = e, ScheduledShift = dbScheduledShift,
                        IsLocked = scheduledShiftDto.LockedEmployeeIds.Any(lei => lei == e.Id)
                    }).ToList();
        ...
        public Schedule CreateSchedule(CreateScheduleDTO scheduleDto, Employee employee)
        {
            ...
            {
                Name = scheduleDto.Name, NumberOfWeeks = scheduleDto.NumberOfWeeks,
                Organization = employee.Organization, ScheduledShifts = new List<ScheduledShift>(),
        ...
        public IEnumerable<Shift> RolloutSchedule(int scheduleId, RolloutScheduleDTO rolloutDto, Employee employee)
        {
                    ...
                    if (scheduledShift.Day == currentDay)
                    {
                        var shift = new Shift
                        {
                            Start = new DateTime(currentDate.Year, currentDate.Month, currentDate.Day,
                                scheduledShift.Start.Hours, scheduledShift.Start.Minutes, scheduledShift.Start.Seconds),
                            End = new DateTime(currentDate.Year, currentDate.Month, currentDate.Day,
                                scheduledShift.End.Hours, scheduledShift.End.Minutes, scheduledShift.End.Seconds),
                            CheckIns = new List<CheckIn>(),
                            Employee_ = scheduledShift.EmployeeAssignments.Select(ea => ea.Employee).ToList(),
            ...
            var shiftsToRemove =
                schedule.Shifts.Where(s =>
                    s.Start > from && s.End > now &&
                    !s.CheckIns.Any()); // delete all previous rolled out shifts that will take place in the future
            var shiftDaysWithCheckIn =
                schedule.Shifts.Where(s => s.CheckIns.Any())
                    .Select(ss => ss.Start.DayOfYear); // get which days are protected because of existing check ins
            shifts = shifts.Where(s => !shiftDaysWithCheckIn.Contains(s.Start.DayOfYear))
                .ToList(); // remove generated shifts that interferes with protected days
        ...
        public IEnumerable<Preference> CreateOrUpdatePreferences(Employee employee, int scheduleId,
            IEnumerable<PreferenceDTO> preferences)
        {
            var scheduledShifts = _scheduleRepository.Read(scheduleId, employee.Organization.Id).ScheduledShifts
                .Where(s => preferences.Select(p => p.ScheduledShiftId).Contains(s.Id)).ToList();
